...
class WonderPlugin_Slider_View {
	...
	function print_overview() {
		...
		<h2><?php //echo __( 'WonderPlugin Slider', 'wonderplugin_slider' ) . ( (WordPress Slider Free Version == "C") ? " Commercial Version" : " Free Version") . " " . WONDERPLUGIN_SLIDER_VERSION; ?> </h2>
	...
	function print_add_new() {
		...
		if ( !empty($_POST['wonderplugin-slider-save-item-post-value']) && !empty($_POST['wonderplugin-slider-save-item-post']) )
		{
	...
	function save_item_post($item_post) {
		...
		if (isset($ret['success']) && $ret['success'] && isset($ret['id']) && $ret['id'] >= 0) 
		{
			...
			echo "<div class='updated'><p>To embed the slider into your page or post, use shortcode <b>[wonderplugin_slider id=\"" . $ret['id'] . "\"]</b></p></div>";
			echo "<div class='updated'><p>To embed the slider into your template, use php code <b>&lt;?php echo do_shortcode('[wonderplugin_slider id=\"" . $ret['id'] . "\"]'); ?&gt;</b></p></div>"; 
		}
		else
		{
